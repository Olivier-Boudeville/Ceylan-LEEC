...
{application, leec, [
  ...
  %
  % Note that no JSON parser is to be selected, as the Erlang-builtin 'json' one
  % will be relied upon (previous, 'jsx' could be replaced by 'jiffy' below).
  %
  ...
  % Our current default: no jsx, elli or shotgun.
  {applications, [kernel, stdlib, myriad]},

  {env, []},

  % Flat hierarchy in ebin here:
  {modules, [leec, leec_api, leec_bot, leec_elli_handler, leec_jws, leec_tls, leec_utils]},
